// TSC Raspberry Pi Version - No strokeWeight calls
let p5Instance;
...
function piSketch(p) {
    ...
    p.initializeParameters = function() {
        ...
        p.frameColors = [p.palette[0], p.palette[9]];
        p.margin = p.w * 0.015;
        p.masterSize = p.w * 0.97;
    };
    
    p.generateParameters = function() {
        p.s1 = p.int(p.random(0, 10));
        p.s2 = p.int(p.random(0, 10));
        p.s3 = p.int(p.random(0, 10));
        p.g1 = p.int(p.random(3, 7));
        p.g2 = p.int(p.random(3, 10));
        p.rc2 = p.int(p.random(0, 9));
        p.rc3 = p.int(p.random(0, 9));
    ...
    p.drawCard = function() {
        ...
        // Main card area - NO strokeWeight calls
    ...
    p.drawCenterSymbol = function(actionId) {
        ...
        switch (actionId % 5) {
            case 0: 
                ...
                break;
            case 1: 
                p.rect(centerX - size/2, centerY - size/2, size, size); 
                break;
            case 2: 
                p.triangle(centerX, centerY - size/2, centerX - size/2, centerY + size/2, centerX + size/2, centerY + size/2); 
                break;
            case 3: 
